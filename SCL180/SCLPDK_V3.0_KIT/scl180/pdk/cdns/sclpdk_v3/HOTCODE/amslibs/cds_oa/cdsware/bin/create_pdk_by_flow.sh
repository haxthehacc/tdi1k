...
set techver = `grep -w $flow $flows_file | awk '{print $3}' | sed 's/_/ /g' | awk '{print $1}'`
if ($ver == "") then
    echo
    echo "Error: the flow 'flow' is not defined in the '$flows_file' or the format is not supported"
    echo
...
else if ($tech_name == "ts18scl") then
    set tech_lib = "ts018_scl_prim"
else if ($tech_name == "ts18sl") then
    set tech_lib = "ts018_prim"
else if ($tech_name == "ts13sl") then
    set tech_lib = "ts013_prim"
else
    echo "Error: the tech_name '$tech_name' is not supported"
endif

if ($tech_name == $techver) then
	set orig_tech_lib = "$hotcode/amslibs/cds_default/cdslibs/$tech_name/devices/devices_oa_${tech_name}_$ver"
else
	set orig_tech_lib = "$hotcode/amslibs/cds_default/cdslibs/$tech_name/devices/devices_oa_${techver}_$ver"
endif
...
if (-d $orig_tech_lib/patch_code) then
 	ln -s $orig_tech_lib/patch_code 
endif
...
if (-d $orig_tech_lib/GR_SHV) then
    ln -s $orig_tech_lib/GR_SHV .
endif
